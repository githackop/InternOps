...
function scanDir(dir) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      if (file.toLowerCase().includes('repository') || file.toLowerCase().includes('repo')) {
        continue;
      }
      ...
      if (content.includes('pool.query') || content.includes('config/db')) {
        ...
        for (let i = 0; i < lines.length; i++) {
          if (lines[i].includes('pool.query') || lines[i].includes('config/db')) {
            console.error(
...
if (leaked) {
  console.error('Lint check failed: pool.query leaked into module routes/logic!');
  process.exit(1);
} else {
